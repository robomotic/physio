...
int main(int argc, char *argv[])
{
	...
	while ( argc > 1 ) {
	  ...
	  if ( argv[argc-1] && (strcmp(argv[argc-1], "-w") == 0) ) {
	    ...
	    {
	      fprintf(stderr, "Usage: %s [-window] [-bpp #] [-w #] [-h #] [-gapsize #] [-sqsize #] [-i] [-freq #]\n", argv[0]);
	      exit(1);
	    }
	}
	...
	for ( i=0; i<screen->h/(gapsize+sqsize); i++ ) {
	  for ( j=0; j<screen->w/(gapsize+sqsize); j++ ) {
	    grid.x = j*(gapsize+sqsize);
	    grid.y = i*(gapsize+sqsize);
	    grid.w = sqsize;
	    grid.h = sqsize;
	    SDL_FillRect(buffer, &grid,  SDL_MapRGB(fmt, fore, fore, fore));
	    grid.x = j*(gapsize+sqsize)+sqsize;
	    grid.y = i*(gapsize+sqsize)+sqsize;
	    grid.w = gapsize;
	    grid.h = gapsize;
	    SDL_FillRect(buffer, &grid,  SDL_MapRGB(fmt, back, back, back));
	  }
	}
